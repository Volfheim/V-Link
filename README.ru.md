...
| Между ПК, безопасный режим | Прямая передача с общим ключом | В новом протоколе материал аутентификации и шифрования выводится раздельно; для постепенного обновления приёмник временно принимает legacy-заголовок v1. Содержимое буфера остаётся открытым, а HTTP-транспорт требует доверенной сети. |
...
**Синхронизация текста включена по умолчанию, изображений — выключена.** Изменения отправляются доступным компьютерам, найденным локальным автообнаружением, а не только выбранному получателю файлов. Буфер передаётся через локальные HTTP-адреса, в том числе при включённой передаче файлов через relay. Безопасный режим использует версионированную аутентификацию с временным legacy fallback, но **не шифрует содержимое буфера обмена**.
