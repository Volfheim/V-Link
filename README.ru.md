...
| Между ПК, безопасный режим | Прямая передача с общим ключом | Исходники v2.4.9 разделяют ключи аутентификации и шифрования файлов и отклоняют незашифрованные загрузки. Совместимость со старыми версиями по умолчанию выключена. Используйте сгенерированный случайный общий ключ и доверенную сеть: метаданные HTTP и токены аутентификации видны в сети и допускают повторное использование. |
...
| Relay, по желанию | Отдельный сервер хранит и пересылает файлы | Для v2 обновите оба ПК и relay-сервер. Relay видит имена, размеры и данные маршрутизации. Reference-сервер требует контроля доступа оператора и HTTPS. Передачи в старом режиме сохраняют уязвимость раскрытия ключа. |

Исходники v2.4.9 готовятся к релизу; ссылка загрузки пока может вести на v2.4.8 с прежней уязвимостью раскрытия ключа. Обновите оба ПК и relay-сервер. Если старое устройство нужно временно сохранить, явно включите **Настройки → Безопасность → Совместимость со старыми версиями (временно)** на обновлённых ПК, только в доверенной сети. После обновления отключите её. Старый режим сохраняет исходную уязвимость; ошибка авторизации не включает его автоматически.

### Синхронизация буфера обмена

**Синхронизация текста включена по умолчанию, изображений — выключена.** Изменения отправляются доступным компьютерам, найденным локальным автообнаружением, а не только выбранному получателю файлов. Буфер передаётся через локальные HTTP-адреса, в том числе при включённой передаче файлов через relay. Безопасный режим использует версионированную аутентификацию с переходом на старый протокол только при явном включении совместимости, но **не шифрует содержимое буфера обмена**.
